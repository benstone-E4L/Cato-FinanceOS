...
# The PTY WebSocket now FAILS CLOSED (see cato/api/ws_auth.py): an app with no
# daemon_token refuses every connection. This test previously relied on the
# unauthenticated path, which encoded fail-open-on-empty as correct.
...
class TestPtyRoutesIntegration(AioHTTPTestCase):
    ...
    async def test_create_session_and_ws_output(self):
        ...
        with (
            ...
            received = []
            async with self.client.ws_connect(f"/ws/pty/{session_id}?token={TEST_DAEMON_TOKEN}") as ws:
