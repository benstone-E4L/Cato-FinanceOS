...
async def test_process_flow_task_broadcasts_activity():
    ...
    gw._broadcast_activity = capture_broadcast
    gw.send = AsyncMock()
    # _process_flow_task fails closed without a gate chain (t20), and building a
    # real one here would register the production tool set into the global
    # _TOOL_REGISTRY and change what LATER test modules observe. Supply a stub.
...
async def test_process_flow_task_broadcasts_idle_on_error():
    ...
    gw._broadcast_activity = capture_broadcast
    gw.send = AsyncMock()
    gw._agent_loop = MagicMock()  # see the note in the test above
