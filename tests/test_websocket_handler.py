...
# The coding-agent WebSocket now FAILS CLOSED: an app with no daemon_token
# refuses every connection, and a connection with no token is refused. These
# tests used to assert the unauthenticated path worked, which encoded the
# fail-open-on-empty defect as correct behaviour; they now authenticate.
TEST_DAEMON_TOKEN = "t21" + "0" * 61
...
def ws_url(path: str) -> str:
    """Authenticated WebSocket URL for the coding-agent endpoint."""
    return f"{path}?token={TEST_DAEMON_TOKEN}"
...
class TestWebSocketHandler(AioHTTPTestCase):
    ...
    async def test_unknown_task_sends_error_and_closes(self):
        ...
        async with self.client.ws_connect(ws_url("/ws/coding-agent/unknown-task-xyz")) as ws:
    ...
    async def test_known_task_sends_status_then_events(self):
        ...
        with patch("cato.api.websocket_handler.invoke_claude_api", return_value=mock_claude), \
            ...
            async with self.client.ws_connect(ws_url(f"/ws/coding-agent/{tid}")) as ws:
                events = []
                try:
                    while True:
...
class TestUIServerIntegration(AioHTTPTestCase):
    ...
    async def test_dashboard_still_serves(self):
        """Still served — but only to a caller holding the daemon token.

        The page injects that token into the HTML, so anonymous access is
        refused (t26 HIGH-2).
        """
        ...
        assert _DAEMON_TOKEN not in await anonymous.text()

        resp = await self.client.get("/", headers={"X-Cato-Token": _DAEMON_TOKEN})
        assert resp.status == 200
