  ...
  const connect = useCallback(() => {
    ...
    import("@xterm/xterm").then((xterm) => {
      import("@xterm/addon-fit").then(({ FitAddon }) => {
        ...
        // The PTY socket authenticates itself after the upgrade and fails
        // closed (cato/api/ws_auth.py). A browser WebSocket cannot set a
        // request header, so the token goes in the query string — it was
        // resolved here and then never sent, so the terminal never
        // authenticated against a daemon that had a token configured.
        const wsUrl =
          `${protocol}://${host}/ws/pty/${sessionId}` +
