...
const DEFAULT_MODELS = new Set([
  "openai/gpt-4o-mini",
  "gpt-4o-mini",
  // legacy minimax slugs — kept so old routing log entries render cleanly
...
export const ChatView: React.FC<ChatViewProps> = ({ wsBase, httpPort, daemonToken, onConnectionStatusChange }) => {
  ...
  const [input, setInput] = useState(() => window.sessionStorage.getItem("cato.pendingPrompt") ?? "");
  ...
  useEffect(() => {
    window.sessionStorage.removeItem("cato.pendingPrompt");
  }, []);
