...
function statusBadgeStyle(ok: boolean): React.CSSProperties {
  return {
    ...
    fontSize: "0.72rem",
    fontWeight: 700,
    background: ok ? "#17255444" : "#7f1d1d44",
    color: ok ? "#93c5fd" : "#fca5a5",
    border: `1px solid ${ok ? "#1d4ed8" : "#ef4444"}`,
  };
}

function FieldRow({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "ok" | "warn" | "error" }) {
  const color = tone === "ok" ? "#93c5fd" : tone === "warn" ? "#fcd34d" : tone === "error" ? "#fca5a5" : "var(--text-primary, #e2e8f0)";
  return (
...
function SwarmSyncTab({ httpPort, wsPort, daemonToken }: { httpPort: number; wsPort?: number; daemonToken?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      {data && (
        <div style={{
          border: `1px solid ${hasFailure ? "#f59e0b" : "#1d4ed8"}`,
          borderRadius: 8,
      ...
      <div style={{
        ...
        {selfTest ? (
          <div>
            <p style={{ color: selfTest.status === "pass" ? "#93c5fd" : "#fca5a5", fontSize: "0.85rem", fontWeight: 700 }}>
              {selfTest.reason}
            </p>
...
function ContradictionsTab({ httpPort }: { httpPort: number }) {
  ...
  const unresolvedColor = (data.unresolved ?? 0) > 0 ? "#f59e0b" : "#38bdf8";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
        {[
          ...
          { label: "Resolved", value: data.resolved, color: "#38bdf8" },
            ...
            {data.domains.map((d) => (
              <tr key={d.domain} style={{ borderBottom: "1px solid var(--border, #222)" }}>
                ...
                <td style={{ padding: "0.4rem 0.6rem", textAlign: "center" }}>
                  <span style={{ color: d.active ? "#38bdf8" : "#6b7280", fontSize: "0.78rem" }}>
          ...
          <tbody>
            {data.corrections.map((c) => (
              <tr key={c.id} style={{ borderBottom: "1px solid var(--border, #222)" }}>
                ...
                <td style={{ padding: "0.4rem 0.6rem", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "#38bdf8" }}>
                  {c.correct_approach}
                </td>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "1.5rem" }}>
        <div style={{ border: "1px solid #38bdf8", borderRadius: 8, padding: "0.75rem 1.25rem", textAlign: "center", background: "var(--surface, #1e1e2e)" }}>
          <div style={{ fontSize: "0.8rem", color: "var(--text-secondary, #aaa)" }}>Confidence Threshold</div>
          <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#38bdf8" }}>{(data.threshold * 100).toFixed(0)}%</div>
        </div>
...
function ContextBudgetTab({ httpPort }: { httpPort: number }) {
  ...
  const SLOT_COLORS: Record<string, string> = {
    ...
    tier1_skill: "#38bdf8", tier1_memory: "#60a5fa",
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
        {data.components.map((c) => (
          <div key={c} style={{ border: "1px solid #38bdf8", borderRadius: 8, padding: "0.5rem 1rem", background: "var(--surface, #1e1e2e)", fontSize: "0.85rem" }}>
            {c}
          </div>
        ))}
      </div>
          ...
          <tbody>
            {data.patterns.map((p) => (
              <tr key={p.habit_id} style={{ borderBottom: "1px solid var(--border, #222)" }}>
                ...
                <td style={{ padding: "0.4rem 0.6rem", textAlign: "right", color: "#38bdf8" }}>{(p.confidence * 100).toFixed(0)}%</td>
