...
def _make_threaded_session(**kwargs):
    """Return an aiohttp.ClientSession that uses the OS DNS resolver.

    aiohttp's default async resolver (aiodns/c-ares) fails to contact DNS
    servers on Windows.  ThreadedResolver delegates to the OS resolver via
    socket.getaddrinfo(), which works correctly on every platform.
    """
    ...
    return _aiohttp.ClientSession(connector=_connector, **kwargs)
...
class WebSearchTool:
    ...
    async def _search_ddg_api(self, query: str) -> list[SearchResult]:
        ...
        async with _make_threaded_session() as session:
