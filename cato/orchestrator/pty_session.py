...
#: The only CLIs that may be spawned in a PTY. Shared with
#: ``cato.api.pty_routes.ALLOWED_CLIS`` so the HTTP check and the spawn check
#: are literally the same set and cannot drift.
ALLOWED_PTY_CLIS = frozenset({"claude", "codex", "gemini"})
...
def build_pty_cmd(cli_name: str) -> list[str]:
    ...
    The allowlist is enforced HERE as well as at the route, because this
    function resolves an arbitrary name through ``shutil.which()`` — without
    its own check, any caller that skipped the route check could spawn any
    executable on PATH. An EMPTY allowlist refuses everything (fail closed).
    """
    name = (cli_name or "").strip().lower()
    if not ALLOWED_PTY_CLIS:
        raise ValueError("PTY spawn refused: the CLI allowlist is empty")
    if name == "cursor":
        raise ValueError("Cursor is one-shot only; no PTY session")
    if name not in ALLOWED_PTY_CLIS:
        raise ValueError(
            f"PTY sessions are limited to {sorted(ALLOWED_PTY_CLIS)}; "
            f"refusing {cli_name!r}"
        )
    return _resolve_cli(name)
