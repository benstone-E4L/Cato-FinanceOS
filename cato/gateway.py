...
from .core.operator_ledger import (
...
#: The ONLY URL schemes a skill may be installed from. FAIL CLOSED: an empty
#: allowlist refuses every install rather than allowing every install.
#: ``ext::``, ``file://``, ``ssh://``, ``git://`` and scp-style ``host:path``
#: are all excluded deliberately — ``ext::`` in particular is a git transport
#: whose documented behaviour is "run this command".
_SKILL_URL_SCHEMES = frozenset({"https", "http"})

#: git config applied to the clone. ``protocol.*.allow=never`` kills the
#: command-executing transports even if a scheme somehow slipped past the
#: allowlist (e.g. via an HTTP redirect or a repo-supplied ``insteadOf``);
#: the empty ``filter.lfs.*`` entries stop a repo-supplied LFS smudge filter
#: from executing at checkout.
_GIT_CLONE_CONFIG: tuple[str, ...] = (
...
#: Environment overrides for the clone: no interactive credential prompt that
#: could hang the daemon, and no LFS smudge on checkout.
_GIT_CLONE_ENV: dict[str, str] = {
...
def _skill_source_problem(url: str) -> "str | None":
    """Return why *url* may not be installed from, or None when it is allowed.

    FAIL CLOSED: anything not positively recognised as an https/http URL with
    ...
    if not url:
        return "empty url"
    if url.startswith("-"):
        # Would be parsed by git as an option, not a repository.
        return "url may not start with '-' (argument injection)"
    if any(c in url for c in ("\n", "\r", "\x00")):
        return "url contains a control character"
    if not _SKILL_URL_SCHEMES:
        return "no skill URL scheme is allowed"
    try:
        parsed = urlparse(url)
    except ValueError as exc:
        return f"unparseable url ({exc})"
    scheme = (parsed.scheme or "").lower()
    if scheme not in _SKILL_URL_SCHEMES:
        return (
            f"scheme {scheme or '(none)'!r} is not allowed; "
            f"use one of {sorted(_SKILL_URL_SCHEMES)}"
        )
...
class Gateway:
    ...
    async def start(self) -> None:
        ...
        # DEPRECATED: _run_cron_scheduler() (CRONS.json poller) is no longer started here.
        # The canonical scheduler is _run_yaml_schedule_daemon() via schedule_manager.py.
        # self._bg_tasks.append(asyncio.create_task(self._run_cron_scheduler(), name="cron-scheduler"))
        self._bg_tasks.append(asyncio.create_task(self._run_yaml_schedule_daemon(), name="yaml-scheduler"))
    ...
    async def _process_flow_task(self, task: dict) -> None:
        ...
        try:
            from .core.scheduled_dispatch import resolve_gate
            from .orchestrator.clawflows import FlowEngine

            # FAIL CLOSED: no gate chain, no dispatch. The engine would
            # otherwise reach raw tool handlers for every step.
            agent_loop = await resolve_gate(self)
            if agent_loop is None:
                logger.error(
                    "Flow task %s refused: gate chain unavailable.", flow_name,
                )
                await self.send(
                    session_id,
                    f"[flow refused: '{flow_name}' cannot run — the "
                    "safety/approval/ledger gate chain is unavailable]",
                    channel,
                )
                return
            engine = FlowEngine(budget=self._budget, agent_loop=agent_loop)
            cap = int(task.get("budget_cap_cents") or 0) or None
    ...
    async def _handle_ws_message(self, ws: Any, raw: str) -> None:
        ...
        if msg_type == "health":
            ...
            if vault_key and value and self._vault is not None:
                try:
                    # LEDGERED: a credential write is the highest-consequence
                    # operator action on this socket. Only the KEY NAME is
                    # recorded — never the value.
                    async def _set_vault() -> None:
                        self._vault.set(vault_key, value)

                    await record_operator_action(
                        tool_name="vault.set",
                        tool_input={"vault_key": vault_key},
            ...
            if not url:
                ...
                # LEDGERED: installing a skill both runs `git clone` and
                # changes what the agent is later instructed by (SKILL.md is
                # injected into the system prompt).
                try:
                    result = await record_operator_action(
                        tool_name="skill.install",
                        tool_input={"url": url},
                        session_id="gateway-ws",
                        run=lambda: self._install_skill_from_url(url),
                        reversibility=0.4,
                    )
                except OperatorLedgerUnavailable as exc:
                    await self._ws_send(ws, {"type": "error", "text": str(exc)})
                    return
                if result:
            ...
            if filename:
                try:
                    # LEDGERED: identity/instruction files feed the system prompt.
                    async def _save_workspace_file() -> None:
                        self._write_workspace_file(filename, content)

                    await record_operator_action(
                        tool_name="workspace.file_write",
                        tool_input={"filename": filename, "bytes": len(content or "")},
                        session_id="gateway-ws",
                        run=_save_workspace_file,
                        reversibility=0.6,
                    )
    ...
    async def _install_skill_from_url(self, url: str) -> "dict | None":
        ...
        SECURITY — ``git clone`` of an arbitrary URL is a code-execution
        primitive, not a download. Three concrete paths existed here:

        * **argument injection.** The URL was passed as a bare positional with
          no ``--`` end-of-options marker, so a "URL" of
          ``--upload-pack=<command>`` was parsed by git as an OPTION and the
          command ran locally. Fixed by ``--`` plus a leading-``-`` reject.
        * **the ``ext::`` transport.** ``ext::sh -c <payload>`` is a git remote
          helper that executes a shell command by design. Fixed by an
          https/http-only scheme allowlist AND ``protocol.ext.allow=never``.
        * **Git LFS smudge filters.** A cloned repo's ``.gitattributes`` plus
          ``.lfsconfig`` can name a custom transfer agent that runs at checkout.
          Fixed by ``GIT_LFS_SKIP_SMUDGE=1`` and an empty ``filter.lfs.*``.

        Submodules were never fetched (no ``--recurse-submodules``); that is
        now explicit rather than incidental. Hooks are not cloned from the
        remote, and ``core.hooksPath`` is pinned to an empty directory anyway.

        This action is still OPERATOR-initiated — it arrives only on the
        token-authenticated gateway WebSocket — so it is not routed through
        ``guarded_action``; it is recorded in the action ledger instead.
        """
        import re
        from pathlib import Path
        ...
        problem = _skill_source_problem(url)
        if problem is not None:
            logger.error("Skill install refused for %r: %s", url[:200], problem)
            return None
        ...
        slug = slug.strip("-")
        if not slug:
            logger.error("Skill install refused: %r yields an empty install directory", url[:200])
            return None
        dest = skills_dir / slug
        # Belt and braces: the slug regex cannot produce a separator, but a
        # dest outside the skills dir would be an rmtree of an arbitrary path.
        try:
            dest.resolve().relative_to(skills_dir.resolve())
        except ValueError:
            logger.error("Skill install refused: %r escapes the skills directory", url[:200])
            return None
    ...
    # DEPRECATED: use YAML scheduler in schedule_manager.py
    # The JSON-based CRONS.json poller below is superseded by _run_yaml_schedule_daemon()
    # which drives SchedulerDaemon from cato/core/schedule_manager.py.
    #
    # This method is NOT started from gateway.start() any longer (see the task list
    # in start() — "cron-scheduler" bg task was removed). It is preserved here so
    # that existing CRONS.json files are not silently orphaned; the `cato cron`
    # CLI commands (add/list/remove/run in cato/cli.py) still read/write CRONS.json
    # and those commands retain their utility for direct prompt-dispatch use-cases
    # that the YAML skill-based scheduler does not cover.
    #
    # To re-enable: add the task back in start() and remove this comment block.
    # Migration path: convert CRONS.json entries to ~/.cato/schedules/*.yaml files.
            ...
            try:
                ...
                for agent_dir in agents_root.iterdir():
                    ...
                    for entry in crons:
                        ...
                        cap_cents = 100
                        try:
                            cap_cents = int(entry.get("budget_cap", 100))
                        except (TypeError, ValueError):
                            cap_cents = 100
                        try:
                            await self._budget.check_and_deduct_usd(
                                min(cap_cents / 100.0, 0.05),
                                label=f"cron:{agent_dir.name}",
                            )
                        except Exception as exc:
                            logger.warning(
                                "Cron %s skipped — budget: %s", agent_dir.name, exc,
                            )
                            continue
                        if announce:
    ...
    async def _run_yaml_schedule_daemon(self) -> None:
        """Run ~/.cato/schedules/*.yaml via SchedulerDaemon + shared dispatch."""
        try:
            from .core.schedule_manager import SchedulerDaemon
            from .core.scheduled_dispatch import dispatch_scheduled_skill
            from .audit import AuditLog
        except ImportError as exc:
            logger.warning("YAML schedule daemon disabled: %s", exc)
            return
        ...
        async def _dispatch(
            ...
            await dispatch_scheduled_skill(
                self,
                skill=skill,
                args=args or {},
                session_id=session_id,
                budget_cap_cents=int(budget_cap or 100),
                channel="cron",
            )

        daemon = SchedulerDaemon(audit_log=audit, dispatch_fn=_dispatch)
        await daemon.start()
        logger.info("YAML SchedulerDaemon started")
        try:
            while True:
    ...
    def _build_agent_loop_sync(self) -> Any:
        ...
        loop._outbound_notify = self._notify_outbound_approval
        return loop

    async def _notify_outbound_approval(self, approval: Any) -> None:
        """Send Telegram inline keyboard for a pending outbound approval."""
        from telegram import InlineKeyboardButton, InlineKeyboardMarkup

        text = (
            f"Approval required ({approval.id})\n"
            f"Tool: {approval.tool_name}\n"
            f"{approval.preview[:300]}"
        )
        markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("Approve", callback_data=f"ns_approve_{approval.id}"),
                InlineKeyboardButton("Deny", callback_data=f"ns_deny_{approval.id}"),
            ]
        ])
        for adapter in self._adapters:
            if type(adapter).__name__ != "TelegramAdapter":
                continue
            if adapter.app is None or adapter.app.bot is None:
                continue
            chat_id = getattr(adapter, "_digest_chat_id", None)
            if chat_id is None:
                # Use last message chat from adapter if available
                continue
            try:
                ...
                logger.warning("outbound approval notify failed: %s", exc)

    async def execute_outbound_approval(self, approval_id: str) -> dict[str, Any]:
        """Approve (if pending) and execute the held tool call exactly once.

        The previous version checked ``row.status`` and dispatched. The row
        stayed ``approved`` afterwards, so every repeat call — a double-tap on
        the Telegram button, a retried POST — re-executed the side effect.

        Redemption now runs through ``execute_approved_tool``, which calls
        ``store.consume``: the signed ticket is verified and marked consumed
        atomically, so the second attempt is refused rather than replayed.
        """
        from .core.outbound_approval import STATUS_PENDING, get_approval_store
        ...
        if row.status == STATUS_PENDING:
            # `approve` issues the signed, single-use ticket that consume()
            # later verifies. `resolve` alone would leave no ticket to redeem.
            if store.approve(approval_id, resolved_by="api") is None:
                return {"ok": False, "error": "approve_failed"}
        await self._ensure_agent_loop()
        if self._agent_loop is None:
            return {"ok": False, "error": "agent_loop_unavailable"}
        result = await self._agent_loop.execute_approved_tool(approval_id)
        # `execute_approved_tool` reports its own refusal in the payload; a
        # refused redemption is not a successful execution.
        ok = True
        try:
