...
class DoctorReport:
    ...
    def _check_budget(self) -> None:
        ...
        try:
            cfg = self._config
            bm = BudgetManager(
                session_cap=cfg.session_cap if cfg else 3.00,  # DEPRECATED — informational only; not enforced
    ...
    def _check_daemon(self) -> None:
        ...
        # Probe the live daemon FIRST. Advice to delete cato.pid/cato.port must
        # never be printed while something is still serving on that port: acting
        # on it lets the next 'cato start' launch a second daemon against the
        # same hash-chained ledger.
        port = self._read_port()
        port_from_file = port is not None
        if port is None:
        ...
        serving = health_ok or self._port_open(port)
        ...
        if _PID_FILE.exists():
            ...
            try:
                ...
                console.print(f"  [red]BAD PID FILE[/red]  invalid pid file: {_PID_FILE}")
                if serving:
                    self._fail(
                        "Daemon pid file is unreadable while a daemon is still serving",
                        f"Do NOT delete {_PID_FILE} yet — stop the live daemon first "
                        f"(cato stop, or taskkill the process listening on port {port}), "
                        f"then start a single daemon: cato start --channel webchat",
                    )
                else:
                ...
                if self._pid_alive(pid):
                    ...
                    console.print(
                        f"  [red]PID/PORT MISMATCH[/red]  PID {pid} is not running, "
                        f"but something is still serving port {port}"
                    )
                    self._fail(
                        "Daemon pid file does not match the process that is serving",
                        f"Do NOT delete {_PID_FILE}/{_PORT_FILE} — that is what allows a "
                        f"second daemon on the same ledger. Identify the live listener "
                        f"(netstat -ano | findstr :{port}), stop it, then: "
                        f"cato start --channel webchat",
                    )
                else:
                    console.print(f"  [red]STALE PID[/red]  PID {pid} is not running")
                    self._fail(
                        "Stale daemon pid file",
                        f"Nothing is listening on port {port}, so it is safe to delete "
                        f"{_PID_FILE} and {_PORT_FILE}, then run: cato start --channel webchat",
                    )
        elif serving:
            console.print(
                f"  [red]NO PID FILE[/red]  port {port} is serving but {_PID_FILE} is missing"
            )
            self._fail(
                "A daemon is running without a pid file",
                f"Do NOT run 'cato start' — it would add a second daemon on the same "
                f"ledger. Find the listener (netstat -ano | findstr :{port}), stop it, "
                f"then start exactly one: cato start --channel webchat",
            )
        else:
        ...
        if not port_from_file:
            console.print(f"  [dim]Port file missing[/dim] — checked configured port {port}")
        elif not serving:
    ...
    def _pid_alive(pid: int) -> bool:
        """Liveness via the shared platform probe (OpenProcess on Windows).

        ``os.kill(pid, 0)`` reported every live Windows daemon as dead, so
        doctor printed "STALE PID" directly above "/health OK" and advised
        deleting the pid/port files — which is exactly what lets a second
        daemon start against the same ledger.
        """
        from cato.platform import pid_alive
