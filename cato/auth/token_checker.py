...
_DEFAULT_ALLOWED_TOOLS = frozenset({
    ...
    # Flows
    "flow.run",
    # Shell — core execution tool; blocked without this when no token is active
    "shell",
    # Genesis — SwarmSync AP2 agent calls; Cato's built-in integration tool
    "genesis",
    # Scheduler/cron skills. Being here means only that this gate is not the
    # one that stops them — exactly like "shell" and "genesis" above. Their
    # real gate is the approval tier in cato/core/approval_policy.py, which
    # holds flow.run (dispatch) for a human ticket. Denying them here instead
    # would make them unapprovable, because authorization runs BEFORE the
    # approval gate that mints the ticket.
    #
    # A retired skill name must be REMOVED from here as well as from the
    # category map below. Leaving it here while dropping its category would be
    # harmless (the category lookup runs first and already refuses), but
    # leaving a stale name in an allowlist is how a later re-mapping quietly
    # resurrects a capability. See t22 (arbitrage.*).
...
_TOOL_CATEGORY_MAP: dict[str, str] = {
    ...
    # Genesis — SwarmSync AP2 agent dispatch
    "genesis":          "api.call",
    # Scheduler/cron skills (cato/core/scheduled_dispatch.py)
    "night_shift.digest":     "schedule.run",
    "night-shift-digest":     "schedule.run",
    "site_services.digest":   "schedule.run",
    "site-services-digest":   "schedule.run",
    "site_services.pulse":    "schedule.run",
    "site-services-inbox":    "schedule.run",
    "schedule.ingest":        "schedule.run",
}
...
class TokenChecker:
    ...
    def check_authorization(
        ...
        if not active_tokens:
            # NOTE: this branch is NOT gated by self._strict.
            # Strict mode disables config-based auto_approved_tools only.
            # _DEFAULT_ALLOWED_TOOLS is the hardcoded baseline and is always
            # honoured regardless of strict mode (see class docstring).
