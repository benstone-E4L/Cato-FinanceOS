...
_FINANCE_OS_HEALTH_FIELDS = {
    "status",
    "db",
    "module_layer_wired",
    "queue_depth",
    "oldest_hold_age_hours",
    "last_xero_sync_at",
...
async def _fetch_finance_os_health() -> dict[str, Any]:
    """Fetch the bounded FinanceOS health contract through the local daemon.

    The desktop webview cannot call FinanceOS directly because the FinanceOS API
    intentionally has no browser CORS surface.  Keeping this bridge server-side
    also prevents the UI from turning a network/CORS failure into a false safety
    status.  Only loopback targets are accepted; this endpoint must never become
    an authenticated SSRF proxy.
    """
    base_url = os.environ.get("E4L_FINANCE_OS_URL", "http://127.0.0.1:3001").rstrip("/")
    parsed = urlparse(base_url)
    if parsed.scheme not in {"http", "https"} or parsed.hostname not in _LOCAL_REMOTES:
        raise ValueError("E4L_FINANCE_OS_URL must target localhost")

    timeout = ClientTimeout(total=4)
    async with ClientSession(timeout=timeout) as session:
        async with session.get(f"{base_url}/health") as response:
            if response.status != 200:
                raise RuntimeError(f"FinanceOS health returned HTTP {response.status}")
            payload = await response.json()

    if not isinstance(payload, dict) or payload.get("status") not in {"ok", "degraded"}:
        raise ValueError("FinanceOS returned an invalid health payload")
    required_types = {
        "db": bool,
        "module_layer_wired": bool,
        "queue_depth": int,
...
# "/" is NOT here: serve_dashboard injects the daemon token into the page, and
# that token unlocks every /api/* route including /api/coding-agent/*, which
# spawns claude/codex/gemini subprocesses. See _mint_dashboard_handoff below
# for how a browser gets in without holding the long-lived token.
...
# The long-lived token never appears in a URL, history entry, or referrer.
...
def _mint_dashboard_handoff(now: float | None = None) -> tuple[str, float]:
    """Create a single-use dashboard ticket. Returns (nonce, ttl_seconds)."""
    current = time.monotonic() if now is None else now
    _prune_dashboard_handoffs(current)
    if len(_DASHBOARD_HANDOFFS) >= _HANDOFF_MAX_OUTSTANDING:
        # Drop the oldest rather than growing without bound.
        oldest = min(_DASHBOARD_HANDOFFS, key=_DASHBOARD_HANDOFFS.__getitem__)
        _DASHBOARD_HANDOFFS.pop(oldest, None)
    nonce = secrets.token_urlsafe(32)
    _DASHBOARD_HANDOFFS[nonce] = current + _HANDOFF_TTL_SECONDS
    return nonce, _HANDOFF_TTL_SECONDS


def _prune_dashboard_handoffs(now: float) -> None:
    for nonce, expires in list(_DASHBOARD_HANDOFFS.items()):
        if expires <= now:
            _DASHBOARD_HANDOFFS.pop(nonce, None)


def _consume_dashboard_handoff(nonce: str) -> bool:
    """Redeem a handoff ticket exactly once. Constant-time on the value."""
    if not nonce:
        return False
    now = time.monotonic()
    _prune_dashboard_handoffs(now)
    for candidate, expires in list(_DASHBOARD_HANDOFFS.items()):
        if secrets.compare_digest(candidate, nonce) and expires > now:
...
# ---------------------------------------------------------------------------
# Host header validation — the actual DNS-rebinding control
# ---------------------------------------------------------------------------
#
# CORS does not help here: once a hostile name resolves to 127.0.0.1 the browser
# treats the daemon's responses as same-origin and the CORS whitelist is never
# consulted. Pinning the Host header is what stops it, and it has to run before
# any handler.
...
def _host_header_allowed(host_header: str) -> bool:
    ...
    if not host_header:
        ...
        # navigation we need to support — fail closed.
        return False
    host = host_header.strip()
    # Strip the port. IPv6 literals are bracketed, so find the last colon
...
@web.middleware
async def host_validation_middleware(request: web.Request, handler):
    """Reject requests whose Host header is not a loopback/Tauri name.

    Runs outermost so a rebound origin never reaches a handler, an auth check,
    or the CORS layer.
    """
    if not _host_header_allowed(request.headers.get("Host", "")):
        logger.warning(
            "Rejected request with non-local Host header on %s %s",
            request.method,
            request.path,
        )
        return web.Response(
            ...
            text="Cato only serves loopback hosts (127.0.0.1, localhost, [::1]).",
        )
...
async def create_ui_app(gateway: Optional[Any] = None) -> web.Application:
    ...
    """
    app = web.Application(
        middlewares=[host_validation_middleware, cors_middleware, auth_token_middleware]
    )
    ...
    async def dashboard_handoff(request: web.Request) -> web.Response:
        """Exchange the daemon token for a single-use browser entry ticket.

        Reaching this handler already proves the caller holds the daemon token
        (auth_token_middleware). Used by `cato dashboard` so the operator's
        browser can open the SPA without the long-lived token ever appearing in
        a URL, history entry, or referrer.
        """
        nonce, ttl = _mint_dashboard_handoff()
        response = web.json_response({"handoff": nonce, "expires_in": ttl})
    ...
    async def health(request: web.Request) -> web.Response:
        ...
        # The `swarmsync_ok` / `swarmsync_status` fields were REMOVED here
        # (t26 MEDIUM-4). They initialised to True and only ever went False when
        # the key STRING was absent — the key was never validated. Live /health
        # reported "swarmsync_ok": true while `cato doctor`'s probe against the
        # same key returned 401 "Invalid or disabled API key". Making the field
        # honest would mean an outbound third-party call on every health poll,
        # for a subsystem the daemon no longer depends on: SwarmSync left the
        # model-execution path in 3bfe37b and Cato calls Anthropic directly.
        # Its remaining consumers (Genesis tool, integration registry,
        # site-services bridge) are covered by `cato doctor`, which actually
        # validates the credential. A green light nothing verifies is worse
        # than no light.
        from cato.audit.recovery import get_last_recovery_scan

        recovery = get_last_recovery_scan()
        if recovery is None:
            # The daemon runs the scan at startup; None means it has not run in
            # this process (e.g. an app assembled directly in a test).
            ledger_recovery: dict[str, Any] = {"scanned": False, "clean": None}
            recovery_ok = True
        else:
            ledger_recovery = {
                "scanned": True,
                "clean": recovery["clean"],
                "scanned_at": recovery["scanned_at"],
                "unresolved_intents": recovery["unresolved_intents"],
                "unreconciled_indeterminate": recovery["unreconciled_indeterminate"],
                "actions": recovery["actions"],
                "indeterminate_actions": recovery["indeterminate_actions"],
                "error": recovery["error"],
            }
            recovery_ok = bool(recovery["clean"])
    ...
    async def compact_session(request: web.Request) -> web.Response:
        ...
        try:
            ...
            if gateway is not None:
                ...
                return web.json_response({"status": "ok", "message": "Context compacted."})
            # No gateway — cannot compact, signal the caller
            return web.json_response(
                {"status": "error", "message": "Gateway unavailable — cannot compact."},
                status=503,
            )
        except Exception as exc:
    ...
    async def run_cron_job_now(request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not result.get("ok"):
                return web.json_response(
                    {"status": "error", "message": result.get("detail", "dispatch failed")},
                    status=400,
                )
            return web.json_response({
                "status": "ok",
                "message": f"Job '{name}' triggered",
    ...
    async def verify_ledger_chain(request: web.Request) -> web.Response:
        """GET /api/audit/ledger/verify — verify causal action ledger hash chain."""
        try:
            import asyncio as _asyncio
            from cato.audit.ledger import verify_chain

            loop = _asyncio.get_running_loop()
            ok, message = await loop.run_in_executor(None, verify_chain)
            return web.json_response({"ok": ok, "message": message})
        except Exception as exc:
            logger.error("verify_ledger_chain error: %s", exc)
            return web.json_response({"ok": False, "error": str(exc)}, status=500)

    async def night_shift_status(request: web.Request) -> web.Response:
        """GET /api/night-shift/status — policy gates, budget, pending approvals."""
        try:
            from cato.core.night_shift_policy import load_night_shift_policy
            ...
            cfg = CatoConfig.load()
            pending = get_approval_store().list_pending(limit=20)
            budget = {}
            if gateway is not None:
                budget = gateway._budget.get_status()
            try:
                ledger_ok, ledger_msg = verify_chain()
            except Exception as ledger_exc:
                ledger_ok, ledger_msg = False, str(ledger_exc)
            return web.json_response({
                "live_outreach_enabled": getattr(cfg, "live_outreach_enabled", False),
                "live_outreach_allowed": policy.live_outreach_allowed,
                "gates": policy.gates,
                "outreach_phase": policy.outreach.get("phase", "manual"),
                "budget": budget,
                "pending_approvals": [
                    {
                        "id": p.id,
                        "tool": p.tool_name,
                        "preview": p.preview[:200],
                        "session_id": p.session_id,
                    }
                    for p in pending
                ],
                "ledger": {"ok": ledger_ok, "message": ledger_msg},
            })
        except Exception as exc:
            logger.error("night_shift_status error: %s", exc)
            return web.json_response({"error": str(exc)}, status=500)

    async def list_outbound_approvals(request: web.Request) -> web.Response:
        from cato.core.outbound_approval import get_approval_store
    ...
    async def approve_outbound(request: web.Request) -> web.Response:
        approval_id = request.match_info.get("approval_id", "")
        if gateway is None:
            return web.json_response({"error": "gateway unavailable"}, status=503)
        data = await gateway.execute_outbound_approval(approval_id)
        return web.json_response(data)

    async def deny_outbound(request: web.Request) -> web.Response:
        from cato.core.outbound_approval import get_approval_store
        approval_id = request.match_info.get("approval_id", "")
        row = get_approval_store().resolve(approval_id, "denied", resolved_by="api")
        if row is None:
            return web.json_response({"ok": False, "error": "not_found_or_resolved"}, status=404)
        return web.json_response({"ok": True, "status": "denied", "id": approval_id})
    ...
    async def save_config(request: web.Request) -> web.Response:
        """POST /config — persist a full config payload via CatoConfig.save()."""
        try:
            ...
            valid_names = {f.name for f in _dc_fields(CatoConfig) if not f.name.startswith("_")}
    ...
    async def finance_os_health(request: web.Request) -> web.Response:
        """Return a truthful, allowlisted view of the local FinanceOS health."""
        try:
            payload = await _fetch_finance_os_health()
            return web.json_response({"connected": True, **payload})
        except Exception as exc:
    ...
    def _effective_autonomy_level() -> float:
        """The autonomy level the enforcing code actually uses.

        Read exactly the way cato/agent_loop.py reads it, so the dashboard and
        ActionGuard can never disagree. It was previously hardcoded 0.5 here.
        """
        cfg = getattr(gateway, "_cfg", None) if gateway is not None else None
        if cfg is None:
            try:
                ...
                return 0.5
        try:
            return float(getattr(cfg, "autonomy_level", 0.5) or 0.5)
        except (TypeError, ValueError):
            return 0.5

    def _irreversibility_check(autonomy: float) -> dict:
        """Row 1 — ActionGuard. Active only if it actually blocks a probe."""
        row = {
            "rule": "Irreversibility check",
            "description": (
                "ActionGuard blocks irreversible actions before dispatch "
                "(cato/agent_loop.py::_check_action_guard)."
            ),
            "active": False,
            "evidence": "",
        }
        try:
            ...
            # api_payment is registered at reversibility 1.0. If the guard does
            # not refuse THAT, it is not enforcing anything.
            decision = ActionGuard().check_before_execute(
                "api_payment", {}, current_autonomy_level=autonomy,
            )
            row["active"] = not decision.proceed
            row["evidence"] = (
                f"probe api_payment -> proceed={decision.proceed} "
                f"({', '.join(decision.applied_checks) or 'no checks applied'})"
            )
        except Exception as exc:
            row["evidence"] = f"guard unavailable: {exc}"
        return row

    def _spend_ceiling_check() -> dict:
        """Row 2 — BudgetManager, which is enforced pre-call in the agent loop.

        Note the honest scope: daily and monthly caps are enforced
        (BudgetManager.check_and_deduct raises BudgetExceeded before the model
        call). The per-session cap is NOT enforced — see cato/budget.py — so it
        is not claimed here.
        """
        row = {
            "rule": "Spending ceiling check",
            "description": (
                "BudgetManager enforces daily and monthly LLM spend caps before "
                "each model call. Per-session caps are not enforced."
            ),
            "active": False,
            "evidence": "",
        }
        try:
            budget = getattr(gateway, "_budget", None) if gateway is not None else None
            if budget is not None:
                status = budget.get_status()
                daily = float(status.get("daily_cap", 0.0) or 0.0)
                monthly = float(status.get("monthly_cap", 0.0) or 0.0)
                evidence = (
                    f"live budget: daily ${status.get('daily_spend', 0.0):.2f}/${daily:.2f}, "
                    f"monthly ${status.get('monthly_spend', 0.0):.2f}/${monthly:.2f}"
                )
            else:
                # No live gateway (standalone UI). Report the configured caps
                # rather than instantiating a BudgetManager, which would touch
                # the on-disk spend state during a read-only status request.
                from cato.config import CatoConfig
                cfg = CatoConfig.load()
                daily = float(getattr(cfg, "daily_cap", 0.0) or 0.0)
                monthly = float(getattr(cfg, "monthly_cap", 0.0) or 0.0)
                evidence = f"configured caps: daily ${daily:.2f}, monthly ${monthly:.2f}"
            row["active"] = daily > 0 or monthly > 0
            row["evidence"] = evidence
        except Exception as exc:
            row["evidence"] = f"budget unavailable: {exc}"
        return row

    def _high_risk_approval_check() -> dict:
        """Row 3 — the outbound approval policy, enforced at dispatch gate 5."""
        row = {
            "rule": "High-risk tool approval",
            "description": (
                "Tools classified financial/irreversible require a human-approved "
                "outbound ticket before execution "
                "(cato/agent_loop.py::_maybe_gate_outbound_tool)."
            ),
            "active": False,
            "evidence": "",
        }
        try:
            ...
            decision = evaluate("integration.action", {"integration": "stripe"})
            row["active"] = decision.requires_approval
            row["evidence"] = (
                f"probe integration.action -> tier={decision.tier}, "
                f"decision={decision.decision} ({decision.reason})"
            )
        except Exception as exc:
            row["evidence"] = f"approval policy unavailable: {exc}"
        return row

    async def action_guard_status(request: web.Request) -> web.Response:
        """GET /api/action-guard/status — report only controls that are enforced.

        Every row's ``active`` bit is the result of exercising the real control,
        not a literal. These three rows previously returned a hardcoded
        ``active: True`` from a handler that consulted nothing.
        """
        autonomy = _effective_autonomy_level()
        try:
            checks = [
                _irreversibility_check(autonomy),
                _spend_ceiling_check(),
                _high_risk_approval_check(),
            ]
            return web.json_response({
                "checks": [c for c in checks if c["active"]],
                "inactive_checks": [c for c in checks if not c["active"]],
                "autonomy_level": autonomy,
            })
        except Exception as exc:
            logger.error("action_guard_status error: %s", exc)
            return web.json_response(
                {"checks": [], "inactive_checks": [], "autonomy_level": autonomy},
                status=500,
            )
    ...
    async def create_flow(request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not re.match(r'^[a-zA-Z0-9_-]+$', name):
                return web.json_response({"error": "invalid name"}, status=400)
            from cato.orchestrator.clawflows import (
                FLOWS_DIR, validate_flow_definition,
            )

            # DEFENCE IN DEPTH: reject unknown step skills at write time. This
            # is NOT the real control — flows can already exist on disk, so
            # FlowEngine._dispatch_step gates every step at run time regardless.
            import yaml as _yaml
            try:
                parsed = _yaml.safe_load(content)
            except Exception as exc:
                return web.json_response(
                    {"error": f"invalid flow YAML: {exc}"}, status=400,
                )
            problems = validate_flow_definition(parsed)
            if problems:
                return web.json_response(
                    {"error": "invalid flow definition", "problems": problems},
                    status=400,
                )
    ...
    async def run_flow_now(request: web.Request) -> web.Response:
        ...
        SECURITY: this is the same X-Cato-Token surface that can WRITE flow
        YAML, so it gets the same treatment the cron path gets — the flow is
        gated as ``flow.run`` (tier ``dispatch``) AND every step inside it is
        gated by ``FlowEngine._dispatch_step``. Gating only the outer flow
        would let one approval authorize an unbounded set of tool calls.
        """
        name = request.match_info.get("name", "")
        try:
            from cato.core.scheduled_dispatch import resolve_gate, run_behind_gate
            from cato.orchestrator.clawflows import FlowEngine
            import re
            if not re.match(r'^[a-zA-Z0-9_-]+$', name):
                return web.json_response({"error": "invalid name"}, status=400)
            ...
            # FAIL CLOSED: no gate chain, no run.
            agent_loop = await resolve_gate(gateway)
            if agent_loop is None:
                return web.json_response({
                    "error": "flow refused: the safety/approval/ledger gate "
            ...
            try:
                ...
                body = {}
            cap = int(body.get("budget_cap_cents") or 500)
            session_id = f"api-flow-{name}"

            async def _run(authorized: bool) -> dict:
                engine = FlowEngine(budget=gateway._budget, agent_loop=agent_loop)
                result = await engine.run_flow(
                    name,
                    trigger_context={
                        "channel": "api", "manual": True, "session_id": session_id,
                    },
            ...
            outcome = await run_behind_gate(
                gateway, gate_tool="flow.run", gate_args={"flow": name},
                session_id=session_id, budget_cap_cents=cap,
                run=_run, action_label="flow",
            )
            if "status" not in outcome:
                # The gate refused before the flow ran.
                return web.json_response({
                    "status": "DENIED",
                    "flow_name": name,
                    "step_outputs": [],
                    "error": str(outcome.get("detail", "denied")),
                    "gate": outcome.get("action"),
                }, status=403)
            return web.json_response({
                "status": outcome["status"],
                "flow_name": outcome["flow_name"],
                "step_outputs": outcome["step_outputs"],
                "error": outcome["error"],
            })
    ...
    app.router.add_get("/api/audit/ledger/verify",       verify_ledger_chain)
    # Night-shift / outbound
    app.router.add_get("/api/night-shift/status",        night_shift_status)
    app.router.add_get("/api/outbound/approvals",        list_outbound_approvals)
    app.router.add_post("/api/outbound/{approval_id}/approve", approve_outbound)
    app.router.add_post("/api/outbound/{approval_id}/deny",    deny_outbound)
    # Config
