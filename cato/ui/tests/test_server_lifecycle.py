...
@pytest.mark.asyncio
async def test_finance_os_health_proxies_allowlisted_payload():
    """The desktop reads FinanceOS health through Cato, never cross-origin."""
    upstream = {
        "status": "ok",
        "db": True,
        "module_layer_wired": True,
        "queue_depth": 2,
        "oldest_hold_age_hours": 1.5,
        "last_xero_sync_at": None,
        "production_write_enabled": False,
        "version": "0.1.0",
        "commit": "abc123",
    }
    with patch("cato.ui.server._fetch_finance_os_health", AsyncMock(return_value=upstream)):
        app = await create_ui_app(gateway=None)
        async with TestClient(TestServer(app)) as client:
            response = await client.get(
                "/api/finance-os/health",
                headers=_auth_headers(),
            )
            assert response.status == 200
            assert await response.json() == {"connected": True, **upstream}


@pytest.mark.asyncio
async def test_finance_os_health_requires_daemon_token():
    """FinanceOS operational metadata is not exposed to anonymous callers."""
    app = await create_ui_app(gateway=None)
    async with TestClient(TestServer(app)) as client:
        response = await client.get("/api/finance-os/health")
        assert response.status == 401


@pytest.mark.asyncio
async def test_finance_os_health_reports_unknown_instead_of_claiming_safe():
    """An unavailable upstream is an explicit unknown state, not a safe state."""
    with patch(
        "cato.ui.server._fetch_finance_os_health",
...
@pytest.mark.asyncio
async def test_finance_os_health_rejects_non_loopback_target(monkeypatch):
    """The authenticated bridge is not an arbitrary outbound proxy."""
    monkeypatch.setenv("E4L_FINANCE_OS_URL", "https://example.com/internal")
    with pytest.raises(ValueError, match="must target localhost"):
        await server_module._fetch_finance_os_health()
...
async def test_dashboard_route_serves_html():
    """GET / must return 200 with HTML content — to an authenticated caller.

    The page body carries the daemon token, so anonymous access is refused
    (t26 HIGH-2). See tests/test_dashboard_token_and_host.py for the full
    disclosure and handoff coverage.
    """
    app = await create_ui_app()
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        assert server_module._DAEMON_TOKEN not in await anonymous.text()
