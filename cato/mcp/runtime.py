...
def _check_mcp_auth(request_headers: dict) -> bool:
    ...
    The expected token is resolved from the encrypted vault.  Missing vault
    state or a missing token denies the request; environment fallback is never
    used.

    **Current limitation:** FastMCP 2.x does not pass raw HTTP request headers
    to tool handler functions. Calls through that unsupported path are denied
    until header access is wired.
    """
    try:
        from cato.vault import get_vault

        expected = str(get_vault().get("CATO_DAEMON_TOKEN") or "")
    except Exception:
        expected = ""
    if not expected:
        return False
...
def create_mcp_server(gateway: Any, config: Any) -> Any:
    ...
    async def cato_list_sessions() -> dict[str, Any]:
        ...
        # Localhost binding is required, and vault-backed token validation fails
        # closed until FastMCP exposes request headers to tool handlers.
