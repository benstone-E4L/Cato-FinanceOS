...
_PRICING: dict[str, tuple[float, float]] = {
    ...
    # Flat-fee tools (Genesis, schedules, flows) — priced per call, not per token
    "genesis-agent":            ( 0.50,       0.10),
    "schedule-reserve":         ( 0.05,       0.00),
    "flow-step":                ( 0.05,       0.00),
}
...
class BudgetPersistenceError(BaseException):
    """The spend could not be durably recorded, so the call must not proceed.

    Derives from ``BaseException`` for the same reason
    :class:`cato.audit.ledger.LedgerError` does: the agent loop wraps the
    budget check in ``except Exception: call_cost = 0.0``
    (cato/agent_loop.py), which silently turned an unwritable budget.json into
    a free, unaccounted model call — and, across a restart, a permanently
    stale daily/monthly total. A cap that cannot be persisted is not a cap.
    """
...
class BudgetManager:
    ...
    def __init__(
        ...
        # Validate what the caller handed us before anything persists it. A
        # CatoConfig object arrived here once (cato/cli.py night-shift status
        # passed the config positionally into `session_cap: float`), which put
        # a non-numeric value into budget.json and made get_status() raise.
        # session_cap is retained for backward compat; informational only.
        self._session_cap = _coerce_cap(session_cap, 3.00, "session_cap (arg)")
        self._monthly_cap = _coerce_cap(monthly_cap, 20.00, "monthly_cap (arg)")
        self._daily_cap = _coerce_cap(daily_cap, 3.00, "daily_cap (arg)")

        self._state = self._load()

        # Override caps from persisted config if they were set differently.
        # Every persisted value is re-validated: budget.json is an ordinary
        # file, and `{"daily_cap": NaN}` is legal JSON that json.loads accepts.
        # NaN silently disables the cap entirely, because `spend + cost > nan`
        # is always False. A cap we cannot read as a finite positive number
        # falls back to the configured one rather than to "no limit".
        if "session_cap" in self._state:
            self._session_cap = _coerce_cap(
                self._state["session_cap"], self._session_cap, "session_cap (budget.json)",
            )
        if "monthly_cap" in self._state:
            self._monthly_cap = _coerce_cap(
                self._state["monthly_cap"], self._monthly_cap, "monthly_cap (budget.json)",
            )
        if "daily_cap" in self._state:
            self._daily_cap = _coerce_cap(
                self._state["daily_cap"], self._daily_cap, "daily_cap (budget.json)",
            )
    ...
    def _load(self) -> dict:
        ...
        try:
            ...
            # Roll over only when the clock has moved FORWARD into a new period
            now_month = _current_month_key()
            if _period_rolled_over(data.get("month_key"), now_month):
                data["monthly_spend"] = 0.0
                data["month_key"] = now_month
                data["monthly_calls"] = 0
            now_day = _current_day_key()
            if _period_rolled_over(data.get("day_key"), now_day):
    ...
    def _save(self) -> None:
        try:
            ...
            # Fail closed and loudly. Swallowing this is how spend stops being
            # counted while calls keep going out.
            logger.critical("Budget state could not be persisted: %s", exc)
            raise BudgetPersistenceError(
                f"budget state could not be written to {self._path}: {exc}"
            ) from exc
    ...
    async def check_and_deduct(
        ...
        *,
        ...
        async with self._lock:
            ...
            now_day = _current_day_key()
            if _period_rolled_over(self._state.get("day_key"), now_day):
                self._state["daily_spend"] = 0.0
                self._state["daily_calls"] = 0
                self._state["day_key"] = now_day

            now_month = _current_month_key()
            if _period_rolled_over(self._state.get("month_key"), now_month):
    ...
    async def check_and_deduct_usd(
        self,
        cost_usd: float,
        *,
        label: str = "misc",
        ...
        """
        Deduct a flat USD amount (schedules, Genesis, flow steps) against daily/monthly caps.

        Uses synthetic token counts derived from cost so existing cap logic applies.
        """
        cost_usd = max(0.0, float(cost_usd))
        if cost_usd <= 0:
            return 0.0
        model = "schedule-reserve" if label.startswith("schedule") else "genesis-agent"
        if label.startswith("flow"):
            model = "flow-step"
        in_price, out_price = _PRICING.get(model, (0.50, 0.10))
        denom = (in_price + out_price) / 1_000_000
        if denom <= 0:
            tokens = 1000
        else:
            tokens = max(100, int(cost_usd / denom))
    ...
    def set_session_cap(self, cap: float) -> None:
        ...
        """
        cap = _coerce_cap(cap, self._session_cap, "session_cap (set)")
    ...
    def set_monthly_cap(self, cap: float) -> None:
        ...
        cap = _coerce_cap(cap, self._monthly_cap, "monthly_cap (set)")
    ...
    def set_daily_cap(self, cap: float) -> None:
        ...
        cap = _coerce_cap(cap, self._daily_cap, "daily_cap (set)")
        self._daily_cap = cap
...
def _coerce_cap(value: object, fallback: float, label: str) -> float:
    """Return *value* as a finite, non-negative spend cap, else *fallback*.

    Rejects NaN and infinity explicitly: both make every ``spend + cost > cap``
    comparison False, which removes the cap without removing the appearance of
    one. Also rejects bools (``True`` is a $1 cap by accident) and anything
    non-numeric.
    """
    import math
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        logger.error(
            "Budget %s is not a number (%r) — falling back to $%.2f", label, value, fallback,
        )
        return float(fallback)
    number = float(value)
    if not math.isfinite(number) or number < 0:
        logger.error(
            "Budget %s is not a finite non-negative amount (%r) — falling back to $%.2f",
...
def _period_rolled_over(stored_key: object, current_key: str) -> bool:
    """True only when time has genuinely moved FORWARD into a new period.

    Rollover used to be `stored != current`, so ANY clock change that altered
    the UTC date — including a jump backwards — zeroed the daily spend counter.
    Toggling the clock therefore granted unlimited spend. Keys are
    zero-padded ``YYYY-MM``/``YYYY-MM-DD``, so a lexicographic compare is a
    chronological compare, and a stored key in the future keeps accumulating.
    """
    if not isinstance(stored_key, str) or not stored_key:
        return True  # no usable history — start a fresh period
    if stored_key == current_key:
        return False
    if stored_key > current_key:
        logger.error(
            "Budget period key %r is AHEAD of the current period %r — the system "
            "clock moved backwards. Keeping the accumulated spend rather than "
            "resetting it.", stored_key, current_key,
        )
        return False
    return True
