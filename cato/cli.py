...
from cato.platform import (
    get_data_dir,
    pid_alive as _platform_pid_alive,
    safe_print,
...
def _pid_alive(pid: int) -> bool:
    ...
    Delegates to :func:`cato.platform.pid_alive`, which uses OpenProcess +
    GetExitCodeProcess on Windows. ``os.kill(pid, 0)`` is not a liveness probe
    there — signal 0 is CTRL_C_EVENT, so it raised OSError for every live
    daemon and made ``cato stop`` orphan the process it was asked to stop.
    """
    return _platform_pid_alive(pid)
...
def _daemon_health_responding(port: int, *, timeout: float = 1.5) -> bool:
    """Return True when something is already answering /health on *port*.

    Used as a belt-and-braces duplicate-start guard: the pid file can be absent
    while a daemon is very much alive, and a second daemon on the same ledger is
    a data-integrity hazard, not a convenience.
    """
    import urllib.error
    ...
    try:
        ...
        # An HTTP error response still proves a server is bound to the port.
        return True
...
async def _bind_http_site_with_fallback(
    ...
    *,
    ...
    log: Any = None,
    allow_port_shift: bool = True,
) -> tuple[Any, int]:
    """Bind an aiohttp site, shifting upward when the preferred port is busy.

    With ``allow_port_shift=False`` the preferred port is retried but never
    abandoned. The daemon uses that mode: shifting to a free port when 8080 is
    occupied is how a second daemon ended up serving the *same* SQLite DBs and
    the same hash-chained ledger as the first one. Transient contention is
    worth a retry; a genuine conflict must fail closed.
    """
    import asyncio
    ...
    for attempt in range(max_attempts):
        candidate_port = preferred_port + (attempt if allow_port_shift else 0)
        try:
            ...
            if attempt == max_attempts - 1:
                if not allow_port_shift and log is not None:
                    log.error(
                        "Port %d is still in use after %d attempts — refusing to "
                        "bind elsewhere. Another daemon is almost certainly live; "
                        "run 'cato stop' (or 'cato doctor') before starting again.",
                        preferred_port,
                        max_attempts,
                    )
                raise
...
def cmd_init() -> None:
    ...
    # Migration notice: CATODESKTOP_BOT_TOKEN is a legacy key name.
    # If the existing vault has CATODESKTOP_BOT_TOKEN but not TELEGRAM_BOT_TOKEN,
    # warn the user. We do NOT auto-copy — manual action is required.
    _existing_vault_path = _CATO_DIR / "vault.enc"
    if _existing_vault_path.exists():
        try:
            _old_vault = Vault(vault_path=_existing_vault_path)
            _old_pw = click.prompt(
                "Existing vault password (for migration check)", hide_input=True
            )
            _old_vault.unlock(_old_pw)
            _has_legacy = bool(_old_vault.get("CATODESKTOP_BOT_TOKEN"))
            _has_canonical = bool(_old_vault.get("TELEGRAM_BOT_TOKEN"))
            if _has_legacy and not _has_canonical:
                safe_print(
                    "\n[DEPRECATION] Your vault contains CATODESKTOP_BOT_TOKEN but not "
                    "TELEGRAM_BOT_TOKEN.\n"
                    "  Please re-enter your Telegram bot token below so it is stored under "
                    "the canonical key (TELEGRAM_BOT_TOKEN).\n"
                    "  The old key CATODESKTOP_BOT_TOKEN will continue to work as a fallback "
                    "but will be removed in a future release.\n"
                )
        except Exception:
            pass  # If vault unlock fails, skip the check silently
    ...
    # DEPRECATED — session_cap is no longer enforced; will be removed in v0.3.0.
    # Mirror daily_cap so legacy config reads land on a sane informational value.
...
def _run_daemon(config: CatoConfig, agent: str, channel: str) -> None:
    ...
    async def _main(cfg: CatoConfig, vlt: "Vault", bdg: BudgetManager) -> None:
        ...
        # Crash recovery runs HERE, at real daemon startup — not on the first
        # chat message. The AgentLoop that used to own this scan is constructed
        # lazily, so an operator who restarted after a crash and sent no message
        # was never told an INTENT was left unresolved. Needs no LLM, and a
        # failure is recorded rather than allowed to block boot.
        from .audit.recovery import run_startup_recovery_scan

        recovery = run_startup_recovery_scan()
        if recovery["error"]:
            safe_print(
                f"LEDGER RECOVERY: the startup scan could not run "
                f"({recovery['error']}). Ledger state is UNVERIFIED — run 'cato doctor'."
            )
        elif not recovery["clean"]:
            safe_print(
                f"LEDGER RECOVERY: {recovery['unresolved_intents']} unresolved INTENT(s) "
                f"and {recovery['unreconciled_indeterminate']} unreconciled action(s) "
                f"from a previous run. See /health -> ledger_recovery, and the "
                f"[CRITICAL] lines in the daemon log."
            )
...
def _request_dashboard_handoff(port: int, token: str, *, timeout: float = 5.0) -> str:
    """Exchange the daemon token for a single-use browser entry ticket."""
    import urllib.request

    req = urllib.request.Request(
        f"http://127.0.0.1:{port}/api/dashboard/handoff",
        data=b"",
        method="POST",
        headers={"X-Cato-Token": token, "Content-Type": "application/json"},
    )
    with urllib.request.urlopen(req, timeout=timeout) as resp:
        payload = json.loads(resp.read().decode("utf-8"))
    handoff = str(payload.get("handoff") or "")
    if not handoff:
        raise ValueError("daemon returned an empty handoff ticket")
    return handoff


@main.command("dashboard")
@click.option("--no-browser", is_flag=True, default=False,
              help="Print nothing and skip opening a browser (exit code only).")
def cmd_dashboard(no_browser: bool) -> None:
    """Open the web dashboard in a browser.

    GET / carries the daemon token, so it is authenticated. This command reads
    the token from the 0600 token file, exchanges it for a ticket that is valid
    once and for 60 seconds, and opens the browser with that ticket — the
    long-lived token never lands in a URL, shell history, or referrer.
    """
    token_path = _CATO_DIR / "daemon.token"
    try:
        token = token_path.read_text(encoding="utf-8").strip()
    except OSError:
        safe_print(f"Cannot read the daemon token at {token_path}. Is Cato installed and started?")
        raise SystemExit(1)
    if not token:
        safe_print(f"The daemon token at {token_path} is empty.")
        raise SystemExit(1)

    port = _discover_http_port()
    try:
        handoff = _request_dashboard_handoff(port, token)
    except Exception as exc:  # noqa: BLE001 — surface any failure the same way
        safe_print(f"Could not obtain a dashboard ticket from http://127.0.0.1:{port}: {exc}")
        safe_print("Is the daemon running? Try: cato status")
        raise SystemExit(1)

    url = f"http://127.0.0.1:{port}/?handoff={handoff}"
    if no_browser:
        return
    import webbrowser

    if webbrowser.open(url):
        # The URL is deliberately not printed: it carries the entry ticket.
        safe_print(f"Opened the Cato dashboard at http://127.0.0.1:{port}/ in your browser.")
    else:
        safe_print("Could not launch a browser. Run 'cato dashboard' from a desktop session.")
        raise SystemExit(1)
...
def cmd_stop() -> None:
    ...
    # terminate_pid handles the platform split: SIGTERM (then SIGKILL) on POSIX,
    # taskkill on Windows where SIGTERM is not deliverable. It returns only once
    # the process is confirmed gone.
    stopped = terminate_pid(pid, timeout=15.0)
    if stopped:
        _PID_FILE.unlink(missing_ok=True)
        _PORT_FILE.unlink(missing_ok=True)
    ...
    # Still alive: keep the pid/port files. Deleting them here is what let the
    # next 'cato start' miss the running daemon and launch a second one against
    # the same ledger.
    safe_print(
        f"Could not stop process {pid} — it is still running. "
        f"The pid and port files were left in place so 'cato start' still sees it."
    )
    raise SystemExit(1)
...
@main.group("night-shift")
def night_shift_cmd() -> None:
    """Night-shift policy, gates, and outbound approval status."""
    pass


@night_shift_cmd.command("status")
def cmd_night_shift_status() -> None:
    """Show gates, budget, pending approvals, and ledger chain status."""
    ...
    cfg = CatoConfig.load()
    # Keyword args, not `BudgetManager(cfg)`: the first positional parameter is
    # `session_cap: float`, so passing the config object put a CatoConfig into
    # a cap field and reported the $3/$20 defaults instead of the configured
    # caps (or raised, on a machine with no budget.json yet).
    budget = BudgetManager(
    ...
    safe_print("Night-shift status")
    safe_print(f"  live_outreach_enabled (config): {getattr(cfg, 'live_outreach_enabled', False)}")
    safe_print(f"  live_outreach_allowed (policy): {policy.live_outreach_allowed}")
    safe_print(f"  outreach phase: {policy.outreach.get('phase', 'manual')}")
    safe_print(f"  G1 proven: {policy.gates.get('g1_manual_loop_proven', False)}")
    safe_print(
        f"  Budget today: ${status['daily_spend']:.2f} / ${status['daily_cap']:.2f}"
    )
    safe_print(f"  Pending approvals: {len(pending)}")
    for p in pending[:10]:
        safe_print(f"    - {p.id} {p.tool_name}")
    safe_print(f"  Ledger: {msg} ({'OK' if ok else 'FAIL'})")


@main.group("outreach")
def outreach_cmd() -> None:
    """Outreach engine paths and credential status (no secret values printed)."""
    pass


@outreach_cmd.command("status")
def cmd_outreach_status() -> None:
    """Show outreach repo path, template version hint, and which keys are configured."""
    from cato.core.outreach_credentials import outreach_credentials_status

    st = outreach_credentials_status()
    safe_print("Outreach status")
    safe_print(f"  engine_root: {st.get('engine_root') or '(not found)'}")
    safe_print(f"  .env file: {st.get('env_file')} ({'exists' if st.get('env_file_exists') else 'missing'})")
    safe_print(f"  templates: {st.get('template_version_hint')}")
    safe_print(f"  postal address set: {st.get('postal_address_set')}")
    safe_print(f"  Brevo SMTP ready: {st.get('brevo_smtp_ready')}")
    safe_print("  keys configured (vault or .env):")
    for key, ok in sorted((st.get("keys_configured") or {}).items()):
        safe_print(f"    {key}: {'yes' if ok else 'no'}")
    safe_print("")
    safe_print("Store secrets: cato vault set BREVO_SMTP_KEY  (and BREVO_SMTP_LOGIN, etc.)")
    safe_print("Docs: docs/brevo-and-outreach-access.md, docs/how-outreach-personalization-works.md")


@main.command("verify-ledger")
def cmd_verify_ledger() -> None:
    """Verify the causal action ledger hash chain in cato.db."""
    from cato.audit.ledger import verify_chain

    ok, msg = verify_chain()
    safe_print(f"Ledger chain: {msg}")
    if not ok:
        sys.exit(1)
...
def cmd_replay(session_id: str, live: bool) -> None:
    ...
    if live:
        if not click.confirm(
            ...
            safe_print("Aborted.")
            return
        # Live replay re-executes recorded tool calls. Each one must go through
        # the same gate chain a fresh call goes through; without it the engine
        # refuses to dispatch rather than re-running the session ungated.
        agent_loop = _build_cli_agent_loop()
        if agent_loop is None:
            safe_print(
                "Refusing live replay: the safety/approval/ledger gate chain "
                "could not be built."
            )
            return
    ...
    engine = ReplayEngine(audit_log=log, agent_loop=agent_loop)
...
def schedule_run(name: str) -> None:
    ...
    async def _run() -> None:
        ...
        vault_path = _CATO_DIR / "vault.enc"
        vlt = Vault(vault_path=vault_path) if vault_path.exists() else None
        cfg = CatoConfig.load()
        bdg = BudgetManager(
            session_cap=cfg.session_cap,
            monthly_cap=cfg.monthly_cap,
        ...
        gw = Gateway(cfg, bdg, vlt)
        from .core.scheduled_dispatch import dispatch_scheduled_skill
        ...
        if result.get("ok"):
            safe_print(
                f"Schedule {name!r} fired (skill={sched.skill!r}, action={result.get('action')})."
            )
        else:
            safe_print(f"Schedule {name!r} failed: {result.get('detail')}")
...
def _build_cli_agent_loop():
    """Build an AgentLoop so in-process CLI dispatch runs the full gate chain.

    Returns None if it cannot be built; callers must then REFUSE to dispatch
    rather than falling back to raw tool handlers.
    """
    try:
        from cato.agent_loop import AgentLoop
        from cato.budget import BudgetManager as _BM
        ...
        return AgentLoop(
            config=cfg, budget=budget, vault=vault,
            memory=MemorySystem(agent_id=cfg.agent_name),
            context_builder=ContextBuilder(max_tokens=cfg.context_budget_tokens),
        )
    except Exception as exc:
        import logging as _logging
        _logging.getLogger("cato").error(
            "Could not build the CLI gate chain: %s", exc,
        )
        return None
...
def flow_run(name: str) -> None:
    ...
    if agent_loop is None:
        safe_print(
            "Refusing to run the flow: the safety/approval/ledger gate chain "
            "could not be built. An ungated flow step is exactly the bypass "
...
from cato.cli_canary import register_canary_commands

register_canary_commands(main)
