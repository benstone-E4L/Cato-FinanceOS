...
from cato.core.approval_policy import take_execution_grant
...
_INTEGRATIONS: tuple[IntegrationSpec, ...] = (
    ...
    IntegrationSpec(
        ...
        vault_keys=("TELEGRAM_BOT_TOKEN", "CATODESKTOP_BOT_TOKEN"),
        env_keys=("TELEGRAM_BOT_TOKEN", "CATODESKTOP_BOT_TOKEN"),
...
async def integration_action(request: web.Request) -> web.Response:
    """POST /api/integrations/{integration}/actions/{action} — plans unless a ticket authorized it.

    SECURITY: ``approved`` and ``dry_run`` are NOT read from the request body.
    This endpoint is operator-authenticated (``auth_token_middleware`` in
    ``cato/ui/server.py`` requires a valid ``X-Cato-Token`` on every POST), but
    a caller-supplied ``approved: true`` is still the same defect shape that was
    removed from ``cato/tools/integration_tool.py``: a JSON field reaching a
    live third-party write (Stripe payment links, GitHub repo creation, Vercel
    deployments). Holding the daemon token is authentication, not per-action
    human approval of a specific payload.

    Authorization has exactly two sources, neither expressible as a body field:

      1. a single-use execution grant minted by
         ``OutboundApprovalStore.consume()`` when a human-approved ticket is
         redeemed for this exact tool + argument digest, or
      2. an ``ApprovalContext`` passed by trusted Python code.
    ...
    # Same canonical tool + argument shape the tool path uses, so one approved
    # ticket authorizes one payload regardless of which entry point redeems it.
    grant_args = {
        "integration": integration_id,
        "action": action_name,
        "params": params,
    }
    authorized = take_execution_grant("integration.action", grant_args)

    if not authorized and _requests_live_write(body):
        # The caller asked for a live write without a redeemed ticket. Say so
        # explicitly rather than silently downgrading to a dry run, so the
        # refusal is visible to the operator and in the audit trail.
        return web.json_response({
            "ok": False,
            ...
            "detail": (
                ...
                "approval ticket. 'approved'/'dry_run' in the request body are "
...
def _requests_live_write(body: dict[str, Any]) -> bool:
    ...
    """
    return _as_bool(body.get("approved", False), default=False) or not _as_bool(
        body.get("dry_run", True), default=True
    )
