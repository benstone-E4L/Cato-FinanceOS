...
async def coding_agent_ws_handler(request: web.Request) -> web.WebSocketResponse:
    ...
    # Authenticate — FAIL CLOSED. This endpoint fans out to the claude / codex
    # / gemini / cursor CLIs as real subprocesses, and it is exempt from the
    # X-Cato-Token middleware, so the check here is the only one. It used to be
    # skipped entirely when the app carried no daemon_token; see cato/api/ws_auth.py.
    async def _deny(message: str) -> None:
        await ws.send_str(_serialize_event("error", {"message": message, "model": None}))

    if not await authenticate_ws(
        request, ws, endpoint="/ws/coding-agent", envelope_timeout=5.0, on_deny=_deny,
    ):
        return ws
    ...
    # The fan-out spawns real CLI subprocesses with an operator-supplied
    # prompt. It is not approval-gated (the operator holding the daemon token
    # IS the approver), so it is LEDGERED instead — INTENT before any process
    # starts. See cato/core/operator_ledger.py for why gate vs ledger.
    async def run_models_recorded() -> None:
        await record_operator_action(
            tool_name="coding_agent.fanout",
            tool_input={"task_id": task_id, "task": task_text, "models": enabled_models},
            session_id=task_id,
            run=run_models,
            reversibility=0.7,
        )
    ...
    models_task    = asyncio.create_task(run_models_recorded())
    ...
    try:
        ...
        logger.error("Coding-agent fan-out refused for task %s: %s", task_id, exc)
        done_event.set()
        heartbeat_task.cancel()
        if not ws.closed:
            await ws.send_str(_serialize_event("error", {
                "message": str(exc), "model": None,
            }))
            await ws.close()
        return ws
    except asyncio.TimeoutError:
