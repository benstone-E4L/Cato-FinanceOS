...
def _sanitize_inputs(inputs: dict) -> dict:
    """Redact credentials from inputs before logging — recursively.

    The previous implementation only inspected top-level dict keys, so
    ``{"headers": {"authorization": "Bearer ..."}}`` was persisted verbatim.
    This now delegates to :func:`cato.audit.ledger.redact`, which walks nested
    dicts and lists, scrubs secret-shaped substrings out of free text, and is
    depth-limited against cyclic payloads.
    """
    if not isinstance(inputs, dict):
        return {}
    from .ledger import redact
    cleaned = redact(inputs)
    # `redact` returns a placeholder string if the structure is too deep to
    # walk; the audit row must still be a dict.
    if not isinstance(cleaned, dict):
        return {"_redacted": str(cleaned)}
    return cleaned
