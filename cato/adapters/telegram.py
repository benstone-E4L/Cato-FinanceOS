...
class TelegramAdapter(BaseAdapter):
    ...
    def __init__(self, gateway: "Gateway", vault: "Vault", config: "CatoConfig") -> None:
        ...
        self._digest_chat_id: int | None = None
    ...
    async def start(self) -> None:
        ...
        self.app.add_handler(
            CallbackQueryHandler(
                self._cb_night_shift_approval,
                pattern=r"^ns_(approve|deny)_[a-f0-9]+$",
            )
        )

        # Site-services permit arbitrage (draft / send / match)
        self.app.bot_data["cato_gateway"] = self.gateway
        from ..tools.site_services_bridge import register_site_services_callbacks

        register_site_services_callbacks(self.app)
        ...
        await self.app.bot.set_my_commands([
            ...
            BotCommand("approvals", "Pending outbound approvals"),
            BotCommand("digest",   "Night-shift status digest"),
        ])
    ...
    async def _cmd_today(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            from cato.model_policy import TaskDescriptor, TaskType  # noqa: PLC0415
            messages = [{"role": "user", "content": prompt}]
            descriptor = TaskDescriptor(
                task_type=TaskType.DRAFT_CORRESPONDENCE,
                task_key="telegram:today_briefing",
            )
            _model, msg, _decision = await self._router.complete_message(
                messages, descriptor,
            )
    ...
    async def _cmd_approvals(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        from cato.core.outbound_approval import get_approval_store

        pending = get_approval_store().list_pending(limit=15)
        if not pending:
            await update.message.reply_text("No pending outbound approvals.")
            return
        lines = [f"Pending ({len(pending)}):"]
        for p in pending:
            lines.append(f"• {p.id} — {p.tool_name}: {p.preview[:80]}")
        await update.message.reply_text("\n".join(lines)[:4000])

    async def _cmd_digest(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        from cato.core.night_shift_digest import build_night_shift_digest

        if update.effective_chat is not None:
            self._digest_chat_id = update.effective_chat.id
        text = await build_night_shift_digest(self.gateway._budget)
        await update.message.reply_text(text[:4000])

    async def _cb_night_shift_approval(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE,
    ) -> None:
        query = update.callback_query
        if query is None:
        ...
        data = query.data or ""
        match = re.match(r"^ns_(approve|deny)_([a-f0-9]+)$", data)
        if not match:
            await query.edit_message_text("Unknown night-shift action.")
            return
        ...
        if action == "deny":
            row = store.resolve(approval_id, "denied", resolved_by="telegram")
            if row is None:
        ...
        row = store.resolve(approval_id, "approved", resolved_by="telegram")
        if row is None:
            await query.answer("Already resolved or missing.", show_alert=True)
            return
        await query.edit_message_reply_markup(reply_markup=None)
        result = await self.gateway.execute_outbound_approval(approval_id)
        ok = result.get("ok", False)
        detail = str(result.get("result", result.get("error", "")))[:500]
        await query.message.reply_text(
            f"Approved {approval_id}. {'OK' if ok else 'FAILED'}\n{detail}"
        )
        ...
        if self._router is not None:
            ...
            try:
                ...
                descriptor = TaskDescriptor(
                    ...
                    task_key="telegram:capture_note",
                )
                _model, msg, _decision = await self._router.complete_message(
                    messages, descriptor,
                )
